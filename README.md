...
- [Задача](#задача)
- [Экран Feed (лента)](#экран-feed-лента)
  - [Список постов](#список-постов-аватар-автора-имя-превью-текста-обложка-поста-счётчик-лайков-и-комментариев)
  - [Таб-фильтр](#таб-фильтр-над-списком-все--бесплатные--платные)
  - [Переход на детальный экран](#tap-на-пост--переход-на-детальный-экран)
  - [Курсорная пагинация](#курсорная-пагинация-подгрузка-при-скролле-вниз)
  - [Pull-to-refresh](#pull-to-refresh)
  - [Закрытый пост](#закрытый-пост-tier-paid--заглушка-вместо-текста)
  - [Сообщение об ошибке](#сообщение-об-ошибке-не-удалось-загрузить-публикации-с-кнопкой-повтора)
- [Экран Post Detail](#экран-post-detail)
  - [Полный текст публикации](#полный-текст-публикации-обложка-имя-автора)
  - [Кнопка лайка с анимацией](#кнопка-лайка-с-анимацией-reanimated-2-плавное-увеличение-счётчика--haptic-feedback)
  - [Список комментариев](#список-комментариев-с-lazy-load)
  - [Поле ввода комментария](#поле-ввода--кнопка-отправки-нового-комментария)
  - [Real-time обновления](#real-time-новые-лайки-и-комментарии-появляются-без-перезагрузки-websocket)
- [Архитектурные решения](#архитектурные-решения)
  - [Почему лайки — optimistic update, а комментарии — нет](#почему-лайки--optimistic-update-а-комментарии--нет)
  - [Архитектура WebSocket-интеграции](#архитектура-websocket-интеграции)
...
Синглтон `wsManager` инициализируется из env-переменных при импорте модуля — до монтирования любого компонента.

> **Известные ограничения `WSManager`** (технический долг)
>
> Текущая реализация содержит несколько защитных механизмов, которые намеренно не реализованы в рамках тестового задания, но должны быть закрыты в продакшне:
>
> - **Нет лимита на количество listeners.** `Map<WSEventType, Set<handler>>` не ограничен по размеру. Хуки `useWsLikeUpdated` / `useWsCommentAdded` корректно вызывают `off` в cleanup `useEffect`, поэтому в текущем коде утечек нет. Однако класс не защищён от неправильного использования новыми потребителями — стоит добавить `MAX_LISTENERS_PER_EVENT` с предупреждением в `console.warn`.
>
> - **Нет защиты от параллельных reconnect.** Метод `reconnect()` планирует `setTimeout` без проверки, не запланирован ли уже предыдущий. При быстрой серии `onclose` (например, нестабильная сеть) можно получить несколько параллельных попыток переподключения. Решение — хранить `reconnectTimer` и делать ранний `return`, если он уже установлен.
>
> - **`disconnect()` не сбрасывает `reconnectAttempts`.** После явного отключения и повторного `connect()` backoff начнётся не с нуля. Listeners при `disconnect()` намеренно не очищаются — они должны пережить переподключение.
>
> - **Нет мониторинга.** В продакшне полезен геттер `stats` (количество listeners по типам, `readyState`, счётчик реконнектов) для диагностики через инструменты наблюдаемости. Это позволяет подписываться на события из любого места без передачи инстанса через props или контекст.
